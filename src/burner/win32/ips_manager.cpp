...
static TCHAR* GameIpsConfigName()
{
	...
	_stprintf(szName, _T("config\\ips\\%.8s.ini"), BurnDrvGetText(DRV_NAME));
	return szName;
}
...
static void FillListBox()
{
	...
	if (hSearch != INVALID_HANDLE_VALUE) {
		...
		while (!Done ) {
			...
			if (fp) {
				...
				PatchDesc = GetPatchDescByLangcode(fp, nSelectedLanguage);
	...
	// Expand all branches
	int nNumNodes = nNumNodes = SendMessage(hIpsList, TVM_GETCOUNT, (WPARAM)0, (LPARAM)0);;
	for (int i = 0; i < nNumNodes; i++) {
...
int GetNumActivePatches()
{
	...
	for (int i = 0; i < MAX_ACTIVE_PATCHES; i++) {
		if (_tcsicmp(szActivePatches[i], _T(""))) nActivePatches++;
	}
...
void LoadActivePatches()
{
	...
	if (fp) {
		while (_fgetts(szLine, sizeof(szLine), fp)) {
			...
			_stprintf(szActivePatches[nActivePatches], _T("%s%s\\%s"), szAppIpsPath, BurnDrvGetText(DRV_NAME), szLine);
			nActivePatches++;
		}		
...
static int IpsManagerInit()
{
	...
	SendDlgItemMessage(hIpsDlg, IDC_CHOOSE_LIST, CB_SETCURSEL, (WPARAM)nSelectedLanguage, (LPARAM)0);
	...
	for (int i = 0; i < nNumPatches; i++) {
		if (hSelectHandle == hPatchHandlesIndex[i]) {
			...
			if (fp) {
				...
				sprintf(szTemp, _TtoA(szImageFileName));
				hNewImage = PNGtoBMP_Simple(hIpsDlg, szTemp);
				fclose(fp);
			}
...
static void SavePatches()
{
	...
	if (fp) {
		...
		for (int i = 0; i < nActivePatches; i++) {
			TCHAR *Tokens;
			TCHAR szFileName[MAX_PATH];
			Tokens = _tcstok(szActivePatches[i], _T("\\"));
			while (Tokens != NULL) {
				szFileName[0] = _T('\0');
...
static BOOL CALLBACK DefInpProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	switch (Msg) {
		case WM_INITDIALOG: {
			...
			DWORD dwStyle;
			dwStyle = (DWORD) GetWindowLongPtr (GetDlgItem(hIpsDlg, IDC_TREE1), GWL_STYLE);
			dwStyle |= TVS_CHECKBOXES;
			SetWindowLongPtr (GetDlgItem(hIpsDlg, IDC_TREE1), GWL_STYLE, (LONG_PTR) dwStyle);
		...
		case WM_COMMAND: {
			...
			if (wID == IDC_CHOOSE_LIST && Notify == CBN_SELCHANGE) {
				nSelectedLanguage = SendMessage(GetDlgItem(hIpsDlg, IDC_CHOOSE_LIST), CB_GETCURSEL, 0, 0);
		...
		case WM_NOTIFY: {
			...
			if (LOWORD(wParam) == IDC_TREE1 && pNmHdr->code == NM_DBLCLK) {
				...
				SetWindowLong(hIpsDlg, DWL_MSGRESULT, 1);
			...
			SetWindowLong(hIpsDlg, DWL_MSGRESULT, CDRF_DODEFAULT);
			return 1;
		}
...
int IpsManagerCreate(HWND hParentWND)
{
	...
	FBADialogBox(hAppInst, MAKEINTRESOURCE(IDD_IPS), hParent, DefInpProc);
	return 1;
}
